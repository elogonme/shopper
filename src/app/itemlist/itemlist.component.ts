...
export class ItemlistComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  displayedColumns: string[] = ['done', 'description', 'quantity', 'unit', 'shopId', 'delete'];
  ...
  updateTable(){
    ...
    const length$ = this.dataSource.loadItems(this.shopId, '', this.sort.direction, this.paginator.pageIndex, this.paginator.pageSize);
    ...
    .pipe(
      ...
      tap(() => { if (this.shopId !== 'Shopping List') { length$.subscribe(pages => this.pageLength = pages);
                  } else { this.itemsService.itemsLength$.subscribe( length => this.pageLength = length );
            }
          })
          )
    .subscribe(() => this.dataSource.loadItems(this.shopId, '', this.sort.direction, this.paginator.pageIndex, this.paginator.pageSize));
  }
